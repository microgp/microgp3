/* --------------------------------------------------------- */
/* --- File: CMA-ES.cpp ------------------------------------ */ 
/* --- Based on cmaes.c by Nikolaus Hansen ----------------- */
/* --------------------------------------------------------- */
/*   
...
#include <stdlib.h> /* nullptr, free */
#include <string.h> /* strlen() */
#include <stdio.h>  /* sprintf(), nullptr? */
...
{
  return (cmaes_GetInto(t, s, nullptr));
}
...
*/
{
  ...
  double deltaprinttimefirst = t->firstprinttime ? time(nullptr)-t->firstprinttime : 0; /* time is in seconds!? */
 ...
 */
{
  ...
  if (rgtmp == nullptr) /* was OK in former versions */
    FATAL("cmaes_t:Eigen(): input parameter double *rgtmp must be non-nullptr", 0,0,0);
...
readpara_SupplementDefaults(readpara_t *t)
{
  ...
  if (t->seed < 1) {
    while ((int) (cloc - clock()) == 0)
      ; /* TODO: remove this for time critical applications!? */
    t->seed = (unsigned int)abs(100*time(nullptr)+clock());
  }
