...
void TiXmlDocument::LoadFile( TiXmlEncoding encoding ) throw(std::exception)
{
	// See STL_STRING_BUG below.
	StringToBuffer buf( value );

	this->SetCondenseWhiteSpace(false);

	this->LoadFile( buf.buffer, encoding );
}
...
void TiXmlDocument::LoadFile( const char* filename, TiXmlEncoding encoding )
{
    if(filename == nullptr)
    {
        throw ugp3::ArgumentNullException("fileName",LOCATION);
    }
	...
	if ( file )
	{
		this->LoadFile( filename, file, encoding );
		fclose( file );
	}
	else
	{
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
		throw ugp3::FileException(value, "File cannot be opened.",LOCATION);
	}
}

void TiXmlDocument::LoadFile( const char * filename, FILE* file, TiXmlEncoding encoding )
{
	if (file == nullptr)
	{
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
        throw ugp3::ArgumentNullException("file",LOCATION);
	}
	...
	if ( length == 0 )
	{
        ...
        throw ugp3::FileException(filename, "The document is empty.",LOCATION);
	}
	...
	*/
	...
	if ( fread( buf, length, 1, file ) != 1 ) {
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
        delete [] buf;
		throw ugp3::Exception("Error parsing document.",LOCATION);
	}
	...
	if (  Error() )
    {
        throw ugp3::FileException(filename,
				  "Error parsing document at line " + ugp3::Convert::toString(ErrorRow()) + ", column "+ ugp3::Convert::toString(ErrorCol()), LOCATION
        );
    }
}
