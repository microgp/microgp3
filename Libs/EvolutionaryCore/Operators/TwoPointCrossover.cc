...
TwoPointCrossoverOperator::TwoPointCrossoverOperator()
{ }

void TwoPointCrossoverOperator::generate(const std::vector< Individual* >& parents,
...
{ 
    ...
    // FIXME what if cutPoint1child1 == cutPoint1child2 ?
    // it seems that it defines a slice of size 1 (cf slice1Size = cutPoint2child1 - cutPoint1child1 + 1)
    // Maybe there is a way to get a slice of size 0 and actually do nothing
    if (cutPoint1child1 > cutPoint2child1) {
        std::swap(cutPoint1child1, cutPoint2child1);
    }
    ...
    // step5: swap subgraphs (requested for incoming outer labels), then swap slices
    LOG_DEBUG << "Swapping subgraphs..." << ends;
    bool success =  this->swapSubGraphs(
	    *child1, *child2, 
	    *selectedSubGraph1, *selectedSubGraph2, 
	    cutPoint1child1, slice1Size, cutPoint1child2, slice2Size);
