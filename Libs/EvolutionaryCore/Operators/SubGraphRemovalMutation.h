/***********************************************************************\
|                                                                       |
| SubGraphRemovalMutationOperator.h                                     |
|                                                                       |
...
#ifndef HEADER_UGP3_CORE_SUBBGRAPHREMOVALMUTATIONOPERATOR
#define HEADER_UGP3_CORE_SUBBGRAPHREMOVALMUTATIONOPERATOR
...
namespace ugp3
{
	namespace core
	{
		/** The SubGraph Removal Mutation Operator. It removes one or more subGraphs. */
		class SubGraphRemovalMutationOperator : public MutationOperator
		{
		public:
			SubGraphRemovalMutationOperator();

			/** It generates new individuals. They are saved in outChildren. outChildren may be empty if
				operator is failed. If sigma is > 0.0, two or more subGraphs could be removed, 
    ...
    virtual Category getCategory() { return DEFAULT_ON; }
                ...
                inline bool SubGraphRemovalMutationOperator::hasParameters() const
                {
                  return false;
                }

                inline void SubGraphRemovalMutationOperator::writeXml(std::ostream& output) const
                {}

                inline void SubGraphRemovalMutationOperator::readXml(const xml::Element& element)
                {}
	}
}
